...
class BearASHIndicator extends CachedIndicator<Num> {

    private final SMAIndicator smaIndicator;

    BearASHIndicator(BarSeries series) {
        super(series);
    ...
    protected Num calculate(int index) {
        ...
        return DecimalNum.valueOf(0.5).multipliedBy(price1.minus(price2).abs().minus(price1.minus(price2)));
    }
}
